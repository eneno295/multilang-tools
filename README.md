...
```bash
code --install-extension multilang-tools-0.1.11.vsix
```
...
打包会生成 `.vsix` 安装包，可用于本地安装或发布到 VSCode 市场。
...
2. **打包**  
   ...
   npm run package
   ```  
   该命令会先执行 `npm run compile` 编译 TypeScript，再调用 `vsce package` 生成 VSIX。

3. **生成结果**  
   - 当前目录下会生成 `multilang-tools-<版本号>.vsix`（例如 `multilang-tools-0.1.11.vsix`）。  
   - 可将该文件分发给他人，或通过「从 VSIX 安装」本地安装。

**可选：发布到 VSCode 市场**

- 需先安装 [vsce](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-vsce) 并登录发布者账号。  
- 发布命令：  
  ```bash
  npm run publish
  ```
