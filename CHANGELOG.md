...
- **错误码翻译语言识别**
  - 修复：按完整文件名匹配导致部分环境下新语言被识别为英文的问题
  - 改进：先去掉扩展名再匹配，兼容 `.js` / `.ts` / `.json` 及短文件名（如 `it.js`、`fr.js`）
  - 结果：打包安装后意大利语、法语、德语、荷兰语、阿拉伯语、俄语、韩语、繁体等均能正确翻译

### 文档

- **README 打包说明**
  - 新增：打包步骤说明（安装依赖、执行 `npm run package`、生成 VSIX）
  - 补充：可选发布到 VSCode 市场的说明
...
- **新增多语言翻译支持**
  - 新增：意大利语、法语、德语、荷兰语、阿拉伯语、俄语、韩语、繁体中文（zh-TW）
  - 支持文件名：`it-IT`、`fr-FR`、`de-DE`、`nl-NL`、`ar-SA`、`ru-RU`、`ko-KR`、`zh-TW`
  - 修复：`zh-TW` 在批量翻译中不会被错误识别为 `zh`
...
- **孟加拉语翻译问题**
  - 修复：打包后孟加拉语翻译失败的问题
  - 修改：将孟加拉语语言代码从 `bn-BD` 改为 `bn`（MyMemory API 支持格式）
  - 改进：添加翻译结果检测，如果返回英文但目标语言不是英文，则跳过翻译
  - 结果：打包后的扩展现在可以正确翻译孟加拉语，不会显示错误的英文翻译
